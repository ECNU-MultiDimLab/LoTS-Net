...
def train_supervised_multimodal_ddp(
    ...
    # SequentialLR: warmup → cosine decay
    # warmup_ratio 控制线性 warmup 占总 epoch 比例（0.2 ≈ 前 20% epoch）
    ...
    warmup_scheduler = optim.lr_scheduler.LinearLR(
        optimizer,
        start_factor=1e-3,      # 从 scaled_lr * 1e-3 开始
        end_factor=1.0,
    ...
    best_dice = 0.0
    # ep50-100 窗口内的最佳验证 Dice（用于防止后期过拟合）
    ...
    early_stop_counter  = 0      # 连续未改善 epoch 计数
    should_stop         = False  # 由 rank-0 设置，广播给其他 rank

    if is_main_process():
        os.makedirs(run_save_dir, exist_ok=True)

    for epoch in range(1, args.epochs + 1):
        # 早停广播：rank-0 决定，所有 rank 同步退出，防止 DDP 死锁
        ...
        if is_main_process():
            ...
            # 超过 patience 阈值则标记停止（下一 epoch 开头广播）
