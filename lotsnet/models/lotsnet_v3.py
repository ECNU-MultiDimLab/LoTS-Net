...
class SelfAwareRouter(nn.Module):
    """Self-attention router (no text input), used in V3 ablation."""

    def __init__(self, c_spe, top_k=10):
        super().__init__()
        self.top_k = top_k
    ...
    def forward(self, f_spec_gap):
        scores = self.mlp(f_spec_gap).squeeze(-1)
        _, indices = torch.topk(scores, k=self.top_k, dim=1)
        return indices


class LOTSNET_V3_Selector(nn.Module):
    """V3: Has Queue, Has Selector (Self-Aware), No Text."""

    def __init__(self, router_top_k=10, queue_len=1000, queue_device="cpu", **kwargs):
        super().__init__()
        ...
        self.router = SelfAwareRouter(c_spe=c_spe, top_k=router_top_k)
    ...
    def forward(self, img, text_spec, text_spa, is_sup=True):
        ...
        indices = self.router(f_spec_gap)
        f_spec_for_select = f_spec.transpose(1, 2)
        f_selected = self.selector(f_spec_for_select, indices)

        f_spec_mean = f_selected.mean(dim=1)
