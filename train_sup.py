...
def print_parser_args(parser, args, *, runtime=None):
    """Print every argument registered on *parser* (plus optional runtime fields)."""
    sep = "=" * 60
    print(sep)
...
def main():
    ...
    # 分别计算 val / test 子集的实际 fg/bg 比例，各自作为对应阶段的指标加权权重
    # 避免 CSV 偏置划分时两个子集分布不同却共用同一套权重，导致 Dice(all) 系统性偏差
